...
function UpdateUserModal(props) {
  ...
  useEffect(() => {
    ...
    setRole(props.role);
    setImagePreview(`https://gestion-maintenance.vercel.app/usersImages/${props.image}`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedFile) {
      ...
      const currentImageBlob = await fetch(`https://gestion-maintenance.vercel.app/usersImages/${props.image}`).then(res => res.blob());
      ...
      <form onSubmit={handleSubmit}>
        <Modal.Body>
          ...
          {!selectedFile && !imagePreview && (
            <div className="mb-3">
              <img
                src={`https://gestion-maintenance.vercel.app/usersImages/${props.image}`}
                alt="Current"
                className="img-fluid rounded"
                style={{ width: "200px", height: "200px" }} // Dimensions spécifiques
              />
