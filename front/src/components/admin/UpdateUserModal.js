...
function UpdateUserModal(props) {
  ...
  useEffect(() => {
    ...
    setRole(props.role);
    setImagePreview(`http://localhost:8000/usersImages/${props.image}`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedFile) {
      ...
      const currentImageBlob = await fetch(`http://localhost:8000/usersImages/${props.image}`).then(res => res.blob());
      ...
      <form onSubmit={handleSubmit}>
        <Modal.Body>
          ...
          {!selectedFile && !imagePreview && (
            <div className="mb-3">
              <img
                src={`http://localhost:8000/usersImages/${props.image}`}
                alt="Current"
                className="img-fluid rounded"
                style={{ width: "200px", height: "200px" }} // Dimensions spécifiques
              />
